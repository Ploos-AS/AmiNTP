...
AROS_URL="$(
  grep -oE 'href="[^"]*amiga-m68k-boot-floppy[^"]*"' "$index_html" \
    | head -n 1 \
...
case "$AROS_ARCHIVE" in
  *.lha|*.LHA)
    ...
    ;;
  *.zip|*.ZIP)
    unzip -q "$archive" -d "$OUT_DIR/extracted"
    ;;
  *)
    echo "ERROR: unsupported AROS boot archive format: $AROS_ARCHIVE" >&2
    exit 1
    ;;
esac
...
printf 'AROS_INDEX_URL=%s\nAROS_TARGET=%s\nAROS_ARCHIVE=%s\nAROS_URL=%s\nADF=%s\n' \
  "$AROS_INDEX_URL" "$AROS_TARGET" "$AROS_ARCHIVE" "$AROS_URL" "$OUT_DIR/bootdisk.adf" \
  > "$OUT_DIR/source.txt"
