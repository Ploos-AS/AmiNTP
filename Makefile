...
TARGET := AmiNTP
COMMON_SOURCES := src/main.c src/cli.c src/version.c src/sntp.c src/query.c src/time.c src/sync.c src/arexx_core.c
...
.PHONY: all clean check host-check m1-check m1.3-check m2.1-check m2.2-check m2.3-check m3.1-check native-check
...
m3.1-check: m2.3-check
	@mkdir -p build/host
	$(HOST_CC) $(CPPFLAGS) $(HOST_CFLAGS) -o $(M31_TEST) tests/test_arexx.c src/arexx_core.c src/version.c
	@$(M31_TEST)
	$(HOST_CC) $(CPPFLAGS) $(HOST_CFLAGS) -o $(HOST_TARGET) $(COMMON_SOURCES) src/net_posix.c src/clock_host.c src/rtc_host.c src/arexx_host.c
	@echo "M3.1 ARexx host compile: PASS"
