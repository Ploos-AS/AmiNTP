...
TARGET := AmiNTP
COMMON_SOURCES := src/main.c src/cli.c src/version.c src/sntp.c src/query.c src/time.c src/sync.c
AMIGA_SOURCES := $(COMMON_SOURCES) src/net_amiga.c src/clock_amiga.c src/rtc_amiga.c
...
.PHONY: all clean check host-check m1-check m1.3-check m2.1-check m2.2-check m2.3-check native-check
...
host-check:
	@mkdir -p build/host
	$(HOST_CC) $(CPPFLAGS) $(HOST_CFLAGS) -o $(HOST_TARGET) $(COMMON_SOURCES) src/net_posix.c src/clock_host.c src/rtc_host.c
